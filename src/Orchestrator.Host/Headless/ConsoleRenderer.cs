...
public sealed class ConsoleRenderer : IDisposable
{
    ...
    private void OnEvent(RunEvent evt)
    {
        var line = evt.Kind switch
        {
            ...
            EventKind.StageInvalidated => $"⟲ {evt.StageId} invalidated (because of {evt["because"]}; workspace {evt["workspace"]})",
            ...
            EventKind.ReplanTriggered => $"⇄ re-plan ({(evt["accepted"] == "true" ? "accepted" : "refused")}): {evt["cause"]}{(evt["mode"].Length > 0 ? $"; mode {evt["mode"]}" : string.Empty)}; invalidated [{evt["invalidated"]}]",
