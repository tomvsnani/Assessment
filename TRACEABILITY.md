...
|---|-------------|-------|--------|
| B1 | Working prototype, runnable end-to-end | `dotnet test` green (152+ tests); dashboard and headless host run; live runs reach `review` and loop back correctly. Replay transparently falls back to recorded runs in `runs/` (e.g. `greenfield-20260915-171952` / `greenfield-20260915-163443`) and supports `--replay` / `--replay-of <runId>`. | [x] |
...
| 2026-09-15 | Redirect integration testing, anti-gaming guardrail, and replay fallback. In `greenfield-20260915-171952`, integration tests failed on `AC5` redirect because `WebApplicationFactory` follows 302 redirects by default, exhausting the loop budget. Prompts for `test-designer` and `implementer` updated with explicit `AllowAutoRedirect = false` instruction. `implementer` prompt fortified against endpoint stripping / dummy stubbing. `RunService` and `HeadlessMode` updated with automatic fallback to recorded runs under `runs/` and support for `--replay-of <runId>`. All 152+ tests clean. | A4e, A5, B1, D8, D13 |
