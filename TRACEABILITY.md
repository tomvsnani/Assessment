...
| A4a | Explicit dependency graph with entry/exit gates | `Orchestrator.Core/Workflow/DependencyGraph.cs`, `Stage.cs`, `Gate.cs`; `workflows/*.yaml` | `Orchestrator.Tests/DependencyGraphTests.cs` | [ ] |
| A4b | Sequential + parallel paths with synchronization | `Orchestrator.Core/Engine/Scheduler.cs` | `SchedulerTests.ParallelBranchesJoinBeforeNextStage` | [ ] |
| A4c | Cross-stage context and decision lineage | `Orchestrator.Core/State/EventStore.cs`, `Projections/Lineage.cs` | `runs/<id>/events.jsonl`; lineage section in each scenario walkthrough | [ ] |
| A4d | Human approval checkpoints for high-impact actions | `Orchestrator.Core/Governance/ApprovalGate.cs`; `approval:` entries in `workflows/*.yaml` | recorded approvals/rejections in `recordings/*/decisions.jsonl` | [ ] |
| A4e | Bounded retries, fallback, rollback, safe-stop | `Engine/RetryPolicy.cs`, `Engine/Saga.cs`, `Engine/SafeStop.cs` | `RetryPolicyTests`, `SagaTests.RollbackRunsInReverse`, a real retry in recordings | [ ] |
| A4f | Policy guardrails — security, compliance, change control | `Governance/Policies/NoSecretsPolicy.cs`, `PiiInLogsPolicy.cs`, `SchemaChangeNeedsApproval.cs`, `SegregationOfDuties.cs` | `PolicyTests`; a real policy block in recordings | [ ] |
| A4g | Audit-grade observability and traceability | `Governance/AuditLog.cs` (JSONL), `State/EventStore.cs` | `runs/<id>/audit.jsonl`; `AuditLogTests.EveryDecisionHasActorAndInputHash` | [ ] |
| A4h | Reliability metrics — success rate, retry/rollback frequency, MTTR, E2E latency | `Orchestrator.Core/Metrics/ReliabilityMetrics.cs` | metrics block printed at end of each `sdlc run`; `ReliabilityMetricsTests` | [ ] |
| A4i | Dynamic re-planning when upstream outputs change | `Engine/Coordinator.cs` | `CoordinatorTests.EditedSpecInvalidatesDownstream`; re-plan shown in `docs/scenarios/ambiguous.md` | [ ] |
