...
| A4i | Dynamic re-planning when upstream outputs change | `Engine/Coordinator.cs`, `Workflow/RerunMode.cs` | `ReplanTests` (verify-fails→implement re-run with feedback, fix vs rollback mode, loop budget, upstream-artifact-changed invalidation) | [x] |
...
| D3 | Ultra-low latency | Cache-first redirect, p99 SLO measured. v1 in-memory: 37,946 rps, p99 3.62 ms. **v1 against real Postgres+Redis: 293 rps, p99 683.77 ms, SLO FAIL** — the synchronous click `UPDATE` on one hot row is the bottleneck, which is the measured motivation for the brownfield scenario | `Endpoints/RedirectEndpoint.cs`, `Core/Links/LinkResolver.cs`, `load/LatencyReport.cs` | [x] |
