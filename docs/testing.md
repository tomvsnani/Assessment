...
## Orchestrator — `tests/Orchestrator.Tests` (77 tests)
...
| YAML workflow parsing (gates, retry, fallback, on_failure) | `WorkflowLoaderTests` (4) |
...
| Bounded retries with feedback, fallback agent, exhaustion | `RetryFallbackTests` (4) |
...
| Artifact tag parsing, fence stripping | `AgentLoopAndParsingTests` |
...
provider path (only recordings prove it; see the scenario walkthroughs).

## Shortener — `tests/Shortener.UnitTests` (39) and `tests/Shortener.IntegrationTests` (14)
...
dotnet run --project src/Orchestrator.Host -- run brownfield  # replay a recorded lifecycle end to end
```
